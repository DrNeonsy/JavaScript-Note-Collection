...
Not everything that is `constant`, is neccessarily / automatically `immutable`, because immutability can also be differently defined per language.
:::
...
TypeScript, and therefore JavaScript, uses `const` to declare variables whose value cannot be reassigned.
...
We still have a `const` pointer `ptr2`, that points to the variable `num1` at first.
...
Preferably, use `let` when you want to make changes immutability after the initial declaration.
